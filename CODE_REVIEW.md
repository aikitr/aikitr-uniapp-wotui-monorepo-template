...
### ROOT-14 🔴 `scripts/bump-version.js` — 指向错误的根 manifest
...
### ROOT-15 🟢 `scripts/open-dev-tools.js` — 仍适用（被 apps/free 引用）
...
### ROOT-16 🟡 `scripts/postupgrade.js` — 指向错误布局
...
### ROOT-17 🔴 `scripts/upload-weixin.js` — 多处失效
...
### ROOT-18 🟡 `scripts/fix-imports.js` / `fix-all-imports.js` — 逻辑冲突且无 dry-run
...
### ROOT-19 🟢 `scripts/create-base-files.js` — 脚手架用途，限定目标
- **问题**：写入 `process.cwd()/src/manifest.json`，在根运行会污染孤儿 `src/`；未接入脚本。
- **建议**：明确为脚手架用途，必须传入目标 app 路径参数；或迁移到 `apps/free` 初始化工具。

### ROOT-20 🟢 `scripts/vite-plugin-eruda.js` — 仍适用
- **问题**：`erudaUrl` 默认 CDN 硬编码。
- **建议**：提取为常量/环境变量，避免硬编码。

### vite-plugins/ 目录

### ROOT-21 🟡 `vite-plugins/copy-native-resources.ts` — 依赖未声明
...
### CORE-01 🔴 `http/alova.ts` — 认证失效 bug
...
### CORE-04 🔴 `router/interceptor.ts` — core → ui 反向依赖
- **问题**：`import { tabbarStore } from '@aikitr/ui/components/tabbar/store'`，core 依赖 ui。
...
### CORE-06 🟡 `api/foo.ts` / `api/foo-alova.ts` — 示例/占位文件
...
### UI-01 🔴 `src/index.ts` — 引用不存在的 `./styles`
...
### UI-02 🔴 `components/tabbar/types.ts` — 破坏库隔离
...
### UI-03 🟡 `components/index.ts` — barrel 过度导出 + 命名冲突
...
### UI-04 🟡 `components/tabbar/index.vue` — 鼓包点击硬编码业务行为
...
### UI-05 🟡 `components/tabbar/TabbarItem.vue` — `uiLib` 分支空实现
...
### UI-06 🟡 `components/tabbar/config.ts` — 注释死代码 + 命名混淆
...
### UI-07 🟡 `components/tabbar/store.ts` — 状态风格不统一
...
### UI-08 🟡 `TabbarItem.test.ts` — 不可执行 + 覆盖不足（见 ARCH-07）
...
### UI-09 🟡 `components/tabbar/README.md` — 与代码多处不一致
...
### UI-10 🟢 `package.json`(ui) / `tsconfig.json`(ui) — 小补
...
### FREE-01 🔴 `src/App.vue` vs `src/App.ku.vue` — 双 App 入口，tabbar 可能不显示
- **问题**：`main.ts` 引用 `App.vue`，但 `App.vue` 既无 `<KuRootView />` 也无 `<FgTabbar />`；而 tabbar 策略为 `CUSTOM_TABBAR`（见 `packages/ui/.../tabbar/config.ts`），自定义 tabbar 必须被挂载才会显示，`layouts/default.vue` 仅是 `<slot/>` 空壳，不会渲染 tabbar。→ **自定义 tabbar 大概率不显示，且丢失 uni-ku 的 KuRootView 布局能力**（迁移回归）。`App.ku.vue` 含 `KuRootView`+`FgTabbar`，明显是「真正的」App 壳，却未被引用（死代码）。
- **建议**：确立唯一入口——若采用 uni-ku 方案，`main.ts` 改 `import './App.ku.vue'`（或把 `KuRootView`+`FgTabbar` 合入 `App.vue`），删除另一个；改完后起 H5 肉眼确认 tabbar 可见。**本项属运行时行为，改造前请先 `pnpm dev:free` 验证当前实际表现。**

### FREE-02 🟡 `src/layouts/default.vue` — 空壳布局，方案冲突
...
### FREE-03 🟡 `src/pages/wotui-demo/wotui-demo.vue` — 示例页打入生产包
...
### FREE-05 🟡 `src/pages.json` / `src/manifest.json` / `src/types/*.d.ts` — 生成物被提交且漂移
...
- **建议**：将 `apps/free/src/{pages.json,manifest.json,types/*.d.ts}` 加入 `.gitignore` 并删除已提交副本，单一以 `*.config.ts` 为准。

### FREE-06 🟡 `manifest.config.ts` — 手写 `getMode()` 脆弱
...
### FREE-07 🟡 `pages.config.ts` — 无效 easycom 规则
...
### FREE-08 🟡 `src/static/tabbar/*` — 7 张 PNG 全未使用
...
### FREE-12 🟢 `vite.config.ts`(apps/free) — 小打磨
...
### FREE-16 🟡 `src/App.vue` 与 `App.ku.vue` 调试日志（联动 FREE-01）
- 两文件含 `console.log` 与 `defineExpose({ helloKuRoot })` 等，若合并入口后清理无用导出与日志。
...
| 编号 | 文件 | 建议 | 理由 |
|---|---|---|---|
| SRC-01 | `src/main.ts` | **删除** | 引用根 src 下不存在的 `./http`/`./router`/`./store`/`@/style`，无法编译；free 版已迁 `@aikitr/core` |
| SRC-02 | `src/App.vue` | **删除** | 引用 `@/router/interceptor`、`@/tabbar/store`（根 src 无），free 版已迁包 |
| SRC-03 | `src/App.ku.vue` | **删除** | 引用 `@/tabbar/index.vue`、`./tabbar/store`、`./utils`（根 src 无），free 版已迁 `@aikitr/ui` |
| SRC-04 | `src/env.d.ts` | **删除** | 声明旧变量名（`VITE_SERVER_PORT` 等）与 free 实际 env 不符，无构建引用 |
| SRC-05 | `src/test-setup.ts` | **删除** | 仅被孤儿 `vitest.config.ts` 引用，根无 vitest 依赖/脚本 |
| SRC-06 | `src/uni.scss` | **删除** | free 用 UnoCSS + `@aikitr/ui`，不引用根 scss |
| SRC-07 | `src/pages.json` | **删除** | 由 uni-pages 生成的快照且已损坏（缺 wotui-demo 页与 wd- 规则），free 的生成才是活的 |
| SRC-08 | `src/manifest.json` | **删除** | 生成物快照，活文件在 `apps/free/src/manifest.json` |
| SRC-09 | `src/pages/index/index.vue` | **删除** | 与 `apps/free/src/pages/index/index.vue` 逐字节相同，重复副本 |
| SRC-10 | `src/pages/about/about.vue` | **删除** | 同上，重复副本 |
| SRC-11 | `src/pages/me/me.vue` | **删除** | 同上，重复副本 |
| SRC-12 | `src/style/iconfont.css` | **删除** | 其 `index.scss` 已注释掉对它的 `@import`，free 不引用 |
| SRC-13 | `src/style/index.scss` | **删除** | 仅被废弃的根 `main.ts` 引用 |
| SRC-14 | `src/types/uni-pages.d.ts` | **删除** | 由孤立根 vite 配置生成，未再更新；活类型在 `apps/free/src/types/` |
| SRC-15 | `src/types/auto-import.d.ts` | **删除** | 引用根 `src/hooks`（不存在）的陈旧生成物 |
| SRC-16 | `src/components/.gitkeep` | **删除** | unibest 本地 components 占位，free 组件在 `packages/ui`/自身 `src/components` |
| SRC-17 | `src/uni_modules/.gitkeep` | **删除** | unibest 的 uni_modules 市场插件占位，free 不使用根 uni_modules |
| SRC-18 | `src/`（整目录） | **删除** | 以上全部删除后整目录可移除 |

> 另：根 `vite.config.ts`、`pages.config.ts`、`manifest.config.ts`、`uno.config.ts`、`index.html`、`vitest.config.ts`、`env/` 同为孤儿，建议一并删除（见 `ARCH-01`、`ROOT-05/06/07`）。
...
### 第二阶段：架构收敛
6. `ARCH-05/06` + `CORE-04/05/07/08/09`：收敛三套请求层、解除循环依赖与 core→ui 反向依赖、统一用户类型、合并重复上传实现。
